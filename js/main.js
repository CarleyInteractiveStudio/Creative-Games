...
async function initApp() {
    ...
    const { data: recommendations } = await getRecommendedGames();
...
async function checkUserAuth() {
    ...
    if (session) {
        ...
        if (profileBtn) {
            ...
            const avatarUrl = session.user.avatar_url || session.user.user_metadata?.avatar_url;
            if (avatarUrl) {
...
async function renderCategories() {
    ...
    const list = cats && cats.length > 0 ? cats : CATEGORIES;
...
async function filterByDevice(device) {
    const { data: dbGames } = await getApprovedGames({ device: device });
    if (!dbGames) return;
...
async function renderDeviceSpecificSection(device) {
    try {
        const { data: games } = await getGamesByDevice(device, 6);
...
function mainGamepadLoop() {
    ...
    if (UP) mainFocusIndex = findNearestVertical(mainFocusIndex, -1);
    if (DOWN) mainFocusIndex = findNearestVertical(mainFocusIndex, 1);
